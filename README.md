# 👋 Hey, I'm Prakash Singh

**Systems & Infrastructure Engineer | Automation, Networking, Self-Hosting, Local AI** ⚡

<div style="display: flex; justify-content: space-between; align-items: center;">
<span>Building practical systems across infrastructure, automation, networking, and intelligent software</span>
<br /><br />
<span>
<img src="https://komarev.com/ghpvc/?username=Codiosityy&label=Profile%20Views&color=8b5cf6&style=flat-square" alt="Profile Views">
<img src="https://img.shields.io/github/followers/Codiosityy?style=flat-square&color=8b5cf6" alt="Followers">
</span>
</div>

---

## 📍 About

I'm a B.Tech student focused on systems engineering, infrastructure, automation, networking, and practical software design.

I enjoy building reliable systems that solve real problems — from self-hosted cloud infrastructure and Docker deployments to local AI pipelines, visualization tools, automation workflows, and cross-platform utilities. Most of my work revolves around understanding how systems behave under the hood and turning that into scalable, usable solutions.

I like working across different layers of technology, whether that means infrastructure, backend logic, developer tooling, or interface design. Hackathons and rapid prototyping have shaped how I build — I solve constrained problems, learn quickly, and ship complete systems from scratch.

---

## 🏆 Hackathon Results

| Event | Result |
|---|---|
| VisualSim (Mirabilis Design) | 🥇 1st Place |
| KnowledgeQuarry (Delhi University) | 🥈 2nd Place |
| DeepTech (IESA) | 🏅 Grand Finalist |

---

## 🛠️ Infrastructure & Systems

<div align="left">

### Core Stack
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white)

### Networking & Infrastructure
![Self-Hosting](https://img.shields.io/badge/Self_Hosting-2B2D42?style=for-the-badge)
![Nextcloud](https://img.shields.io/badge/Nextcloud-0462D1?style=for-the-badge&logo=nextcloud&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=for-the-badge&logo=nginx&logoColor=white)
![Reverse Proxy](https://img.shields.io/badge/Reverse_Proxy-00A98F?style=for-the-badge)
![DDNS](https://img.shields.io/badge/DDNS-FF6B6B?style=for-the-badge)

### Automation & Tooling
![Ollama](https://img.shields.io/badge/Ollama-446B9C?style=for-the-badge)
![llama.cpp](https://img.shields.io/badge/llama.cpp-8B5CF6?style=for-the-badge)
![LangChain](https://img.shields.io/badge/LangChain-5B5C5E?style=for-the-badge)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white)

### AI / ML Systems
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![Computer Vision](https://img.shields.io/badge/Computer_Vision-4B0082?style=for-the-badge)
![Edge AI](https://img.shields.io/badge/Edge_AI-00A98F?style=for-the-badge)
![Stable Diffusion](https://img.shields.io/badge/Stable_Diffusion-FF6F00?style=for-the-badge)

### Web & Backend
![Node.js](https://img.shields.io/badge/Node.js-339939?style=for-the-badge&logo=node.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge&logo=tailwind-css&logoColor=white)

### Data & Config
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![YAML](https://img.shields.io/badge/YAML-CB171E?style=for-the-badge&logo=yaml&logoColor=white)
![JSON](https://img.shields.io/badge/JSON-000000?style=for-the-badge&logo=json&logoColor=white)

</div>

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api?username=Codiosityy&show_icons=true&theme=radical&hide_border=true&count_private=true" alt="GitHub Stats" height="180">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=Codiosityy&layout=compact&theme=radical&hide_border=true" alt="Top Languages" height="180">
</div>

---

## 🔧 Featured Projects

### [Forsaken-Apex](https://github.com/Codiosityy/Forsaken-Apex) ⭐ — *DeepTech Grand Finalist*

Production-ready semiconductor defect detection pipeline using MobileNetV2 + SE Attention. Trained on 9,000+ images with synthetic data augmentation via Stable Diffusion for class balancing. Achieved 86.6% accuracy on INT8 edge-deployed model for NXP hardware.

**Stack:** TensorFlow, Keras, Computer Vision, Edge AI, Synthetic Data Generation

---

### [DualZen](https://github.com/Codiosityy/DualZen)

Cross-platform browser workspace synchronization system using Python native messaging + browser extension. Syncs pinned tabs, tab groups, and workspace organization via Google Drive across dual-boot machines. Implemented full backup/restore pipeline with session file encoding.

**Stack:** Python, JavaScript, Browser Extensions, Google Drive API, Native Messaging

---

### [VisualizeSim](https://github.com/Codiosityy/VisualizeSim) — *1st Place Hackathon Winner*

Simulation visualization system for interpreting complex datasets. Designed interactive UI for extracting meaningful insights from simulation outputs. Won 1st place at Mirabilis Design VisualSim Hackathon.

**Stack:** Python, Data Visualization, UI/UX

---

### [openenv-vendor-compliance](https://github.com/Codiosityy/openenv-vendor-compliance)

OpenEnv environment for deterministic vendor onboarding and compliance review workflow. Built with 3 graded tasks for compliance verification and Docker-based deployment for reproducibility.

**Stack:** Python, Dockerfile, Compliance Workflow

---

### [QuarryFlow-Crossing](https://github.com/Codiosityy/QuarryFlow-Crossing) — *2nd Place (Delhi University)*

Logic-driven system solving structured crossing problems under constraints. Built for hackathon environment with focus on algorithmic correctness and efficiency. Placed 2nd at Delhi University's KnowledgeQuarry.

**Stack:** Python, Algorithm Design

---

## 🎯 Currently Working On

- Home lab infrastructure expansion
- Local LLM benchmarking and optimization
- Docker Compose orchestration for personal services
- Network security hardening for self-hosted deployments

---

## 📍 Location

📍 **India** · 🕤 UTC+5:30

---

## 🤝 Connect

<div align="left">
<a href="https://github.com/Codiosityy"><img src="https://img.shields.io/badge/GitHub-100000?style=flat-square&logo=github&logoColor=white" alt="GitHub"></a>
<a href="https://www.linkedin.com/in/codiosity/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
<a href="mailto:singhprakaash334@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" alt="Email"></a>
</div>

---

> "Build reliable systems. Ship complete solutions. Iterate fast."
